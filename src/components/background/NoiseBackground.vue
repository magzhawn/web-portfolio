...
onMounted(() => {
  ...
  `;
    ...
    // Hash function instead of permutation table
    float hash(float n) {
  ...
  `;
    ...
    function render(gl: WebGLRenderingContext, canvas: HTMLCanvasElement) {
        ...
        const r = parseFloat(styles.getPropertyValue('--noise-r') || '30') / 255;
        const g = parseFloat(styles.getPropertyValue('--noise-g') || '30') / 255;
        const b = parseFloat(styles.getPropertyValue('--noise-b') || '30') / 255;
